...
export default function MapPage() {
  ...
  // Helper: check content moderation via Gemini
  async function checkModeration(title: string, caption: string, imageUrl?: string): Promise<boolean> {
    try {
      ...
      const { appropriate } = await res.json();
      return appropriate;
    } catch {
      return true; // fail open if moderation API is down
    }
  }
  ...
  async function handleDrop() {
    ...
    setDropping(true);
    try {
      const ok = await checkModeration(dropTitle, dropCaption);
      if (!ok) {
        alert("⚠️ Your content was flagged as inappropriate.");
        setDropping(false);
        return;
      }
  ...
  async function handleAddPost() {
    ...
    setAddingPost(true);
    try {
      const ok = await checkModeration("post", addCaption || "no caption");
      if (!ok) {
        alert("⚠️ Your content was flagged as inappropriate.");
        setAddingPost(false);
        return;
